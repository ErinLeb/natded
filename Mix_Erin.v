...
(** Validity of a derivation : is it using correct rules
    of classical logic (resp. intuitionistic logic and minimal) ? *)
...
Definition valid_deriv_step logic '(Rule r s ld) :=
  ...
  | Ax,     (Γ ⊢ A), [] => list_mem A Γ
  | Tr_i,   (_ ⊢ True), [] => true
  | Fa_e,   (Γ ⊢ _), [s] => ((logic =?  J ) || (logic =? K)) &&& (s =? (Γ ⊢ False) )
...
Proof.
 destruct lg. 
  - (*K*) Abort. (*rewrite Pr_minimal_intuit. apply Pr_intuit_classic. 
  - (*J*) apply Pr_minimal_intuit.
  - (*M*) trivial. 
Qed.*)
